/**
 * Official Robinhood feather mark (source: public/Robinhood Logos/*.svg).
 * Inlined so it can be colored per context:
 *  - default: currentColor (inherits text color)
 *  - brand:   var(--brand-robinhood) → black on light theme, neon on dark
 * Used to identify the Robinhood Chain network and the HOOD stock-token
 * asset only — see the trademark note in the footer.
 */
export default function RobinhoodMark({
  size = 16,
  brand = false,
  className,
}: {
  ...
  // Native aspect ratio is 115.87 × 149.53 (taller than wide).
  const width = size * (115.87 / 149.53);
  return (
    <svg
      width={width}
      height={size}
